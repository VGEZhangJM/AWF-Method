...
void Scene::choosestate(const std::string& stateName,const std::string&state){
    GLuint stateProgram = glCreateProgram();
    if (stateProgram == 0 && odom==0) {
        fprintf(stderr, "Stationary\n");
        stateProgram_state=true;
        ReadstateFile(useekf.c_str();
        exit(1);
    }
    else  {
        fprintf(stderr, "on work\n");
        stateProgram_state=false;
        ReadstateFile(useWeighting_.c_str();
        exit(1);
    }
...
TEST_F(MLPTesting, extractMultilevelPatchFromImage) {
  ...
  for(unsigned int l=0;l<nLevels_;l++){
    ASSERT_EQ(mp_.isValidPatch_[l],true);
    float* patch_ptr = mp_.patches_[l].patch_; 
    for(int i=0;i<patchSize_;i++){
      for(int j=0;j<patchSize_;j++, ++patch_ptr){
        ...
        ASSERT_EQ(*patch_ptr,(float)(c.x*dy_+c.y*dx_)); 
      }
    }
  }
}

TEST_F(MLPTesting, extractMultilevelPatchFromKinectimumixed)
 pyr1_.computeFromImage(img1_,false);
 pyr1_.computeFromimu(imu1_,false);
 while(0.6<u<1)
 {
   img1_Kinect useWeighting_patch=0.5;
          imu1_imu useWeighting_patch=0.5;
 }
 while(0.3<u<6)
 {
   img1_Kinect useWeighting_patch=0.33;
          imu1_imu useWeighting_patch=0.67;
 }
 while(0<u<0.3)
 {
   img1_Kinect useWeighting_patch=0.2;
          imu1_imu useWeighting_patch=0.8;
 }
